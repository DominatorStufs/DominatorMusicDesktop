                                ...
                                <ScrollArea className="h-[390px] md:px-0 px-4">
                                    <div className="flex flex-col gap-2">
                                        {data.length > 0 && data.map((song) => (
                                            <Link className="w-full hover:bg-secondary/50 border border-border rounded-xl p-3 flex items-center justify-between gap-3 transition" key={song.id} href={`/${encodeURIComponent(normalizeId(song.id))}`}>
                                                <div className="flex items-center gap-3 min-w-0 flex-1">
                                                    <img src={song.image?.[2]?.url || ""} alt={song.name} className="bg-secondary/50 w-9 h-9 rounded-lg object-cover flex-shrink-0" />
                                                    <div className="text-sm min-w-0">
                                                        <p className="truncate" title={decodeEntities(song.name)}>
                                                            {decodeEntities(song.name)}
                                                        </p>
                                                        <span className="text-muted-foreground text-xs truncate block">
                                                            {decodeEntities(song.artists?.primary?.[0]?.name) || "unknown"}
                                                        </span>
                                                    </div>
                                                </div>
